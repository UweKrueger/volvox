...
llvm::Type* llvm_int_type = nullptr;
llvm::Type* llvm_size_type = nullptr;
llvm::Type* llvm_bool_type = nullptr;
...
volvoxc::FullType* size_type = nullptr;
volvoxc::FullType* integer_type = nullptr;
volvoxc::FullType* f32_type = nullptr;
volvoxc::FullType* interface_type = nullptr;
volvoxc::FullType* interface_ref_type = nullptr;
volvoxc::FullType* va_arg_type = nullptr;
volvoxc::FullType* voidptr_type = nullptr;
...
void init(const llvm::Triple& triple) {
	...
	lex.add_type("float", llvm::Type::getFloatTy(Context), DBuilder ? DBuilder->createBasicType("float", 32, llvm::dwarf::DW_ATE_float) : nullptr);
	...
	llvm::Type* llvm_interface_ref_type = llvm_ptr_type;
...
/*	case llvm::Triple::GNUF32:
	...
	if (!lex.module->globals_table.insert("__LLVM_JIT", is_jit_fv)) {
...
void CallTestFunction(bool immediately = false) {
	...
	if (immediately) {
		...
		size_t lalloc;
		cstr2volvoxstr(volvoxstrTestFunction, lalloc, buf, TestFunction, alloca, 0);
...
int main(int argc, char* argv[]) {
	...
	auto builtin_fn = std::string(volvox_root()) + "/lib/volvox/" + builtin_file_name;
