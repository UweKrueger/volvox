...
llvm::Value* LiteralExprAST::codegen_raw(llvm::Value* target) {
	...
	case llvm::Type::IntegerTyID:
	{
		...
		if (desired_type) {
			if (auto it = llvm::dyn_cast<llvm::IntegerType>(desired_type))
				...
				return handle(target, llvm::ConstantFP::get(desired_type, ft->type_attr & A_signed ? (double)Val.Int : (double)Val.Uint), Loc, ft);
		}
		if (!bw)
			bw = ft->type->getIntegerBitWidth();
		if (bw == 1) // bool - treat anything != 0 as 'true'
			if (Val.Uint)
				return handle(target, Builder->getTrue(), Loc, ft);
			else
	...
	case llvm::Type::PointerTyID:
		...
		return handle(target, Builder->CreateIntToPtr(llvm::ConstantInt::get(llvm_size_type, Val.Uint, false), llvm_ptr_type), Loc, ft);
		// else fallthrough
	default:
		if (ft->type == llvm_string_type) {
			llvm::Value* theString = createStringConst(Val.CStr, Val.Len);
			return handle(target, theString, Loc, ft);
		}
...
llvm::Value* ListExprAST::codegen_raw(llvm::Value* target) {
	...
	if (desired_type) {
		if (auto struct_type = llvm::dyn_cast<llvm::StructType>(desired_type)) {
			...
			auto str_ft = RtType_for_fqname[std::string(struct_type->getName())].ft;
			if (str_ft) {
...
std::pair<PrototypeAST*,llvm::Function*> get_inserter_fn_for_key_type(SourceLocation& Loc, volvoxc::FullType* ft) {
	...
	const char* inserter;
	llvm::Type* key_type = ft->elem_type[0].type;
	unsigned key_type_attr = ft->elem_type[0].type_attr;
	if (key_type == llvm_string_type) // string key type
	...
	return { inserter_proto, getFunction(inserter_proto) };
}
...
llvm::Value* MapExprAST::codegen_raw(llvm::Value* target) {
	...
	auto [ inserter_proto, inserter_fn ] = get_inserter_fn_for_key_type(Loc, ft);
...
llvm::Value* SetExprAST::codegen_raw(llvm::Value* target) {
	...
	auto [ inserter_proto, inserter_fn ] = get_inserter_fn_for_key_type(Loc, ft);
	llvm::Value* set_ref = ((intptr_t)target == -1) ? nullptr : target;
	if (!set_ref) {
		set_ref = CreateEntryBlockAlloca(llvm_map_type);
		FullVar tmp = {
			.val = set_ref,
			.ft = {
				.type = llvm_map_type,
	...
	llvm::Value* ptr = Builder->CreateStructGEP(llvm_map_type, set_ref, 0);
	llvm::Value* count_ref = Builder->CreateStructGEP(llvm_map_type, set_ref, 1);
	Builder->CreateStore(llvm::ConstantPointerNull::get(llvm_ptr_type), ptr);
	llvm::Value* do_replace = CreateEntryBlockAlloca(llvm_ptr_type);
	for (unsigned i=0; i<Elements.size(); i++) {
		Elements[i]->desired_type = ft->elem_type[0].type;
		Builder->CreateStore(llvm::ConstantPointerNull::get(llvm_ptr_type), do_replace);
		llvm::Value* Key = Elements[i]->codegen(true);
		llvm::Value* Value = llvm::Constant::getNullValue(llvm::Type::getInt64Ty(Context));
		Builder->CreateCall(inserter_proto->FT, inserter_fn, std::vector<llvm::Value*>{
				ptr, Key, Value, Builder->getInt32(0), do_replace });
	}
	Builder->CreateStore(getSize(Elements.size()), count_ref);
	if (target && (intptr_t)target != -1)
		return llvm::UndefValue::get(llvm::Type::getVoidTy(Context));
	return Builder->CreateLoad(llvm_map_type, set_ref);
}
...
llvm::Value* LvalueExprAST::codegen_raw(llvm::Value* target) {
	...
	if (V.first && V.second) {
		if (V.first->isSized())
			// Load the value.
			return handle(target, Builder->CreateLoad(V.first, V.second, Name.c_str()), Loc, ft);
		else
	...
	if (V.first) {
		if (auto arr_type = llvm::dyn_cast<llvm::ArrayType>(Struct->ft->type)) {
			...
			if (auto array_ast = dynamic_cast<LvalueExprAST*>(Struct.get()))
				std::tie(arr_ty, arr) = array_ast->codegen_ref(true, true);
			if (!arr && arr_ty)
				arr = Struct->codegen_raw();
			if (!arr) {
				errs() << Loc << ": invalid array for method\n";
				return nullptr;
			}
...
llvm::Value* InterfaceExprAST::codegen_raw(llvm::Value* target) {
	...
	if (expr->ft->type_attr & A_interface) {
		auto inter_face = expr->codegen_raw();
		rttype_ptr = Builder->CreateExtractValue(inter_face, 0);
		llvm::Value* Offset;
		if (ft->InterfaceProtos) {
		...
		rttype_ptr = Builder->CreateIntToPtr(
			Builder->CreateAdd(
				Builder->CreatePtrToInt(rttype_ptr, llvm_size_type), Offset), llvm_ptr_type);
		goto have_rttype;	                   
	} else if (auto array_type = llvm::dyn_cast<llvm::ArrayType>(expr->ft->type)) {
		if (interface_ft) {
	...
	rttype_ptr = getRtType(expr->ft, vtable);
	if (!rttype_ptr) {
...
have_rttype:
	...
	return handle(target, the_struct, Loc, ft);
}
...
llvm::Value* PostfixExprAST::codegen_raw(llvm::Value* target) {
	...
	if (auto int_ty = llvm::dyn_cast<llvm::IntegerType>(OperandV.first)) {
		...
		if (Operand->ft->type_attr & A_atomic) {
			if (Opcode[0] == '+')
				return handle(target, CreateAtomicRMW(llvm::AtomicRMWInst::BinOp::Add, OperandV.second, One), Loc, ft);
			else
				return handle(target, CreateAtomicRMW(llvm::AtomicRMWInst::BinOp::Sub, OperandV.second, One), Loc, ft);
		}
		...
		return handle(target, oldVal, Loc, ft);
	} else if (OperandV.first->isFloatingPointTy()) {
		auto One = Builder->CreateUIToFP(Builder->getInt32(1), OperandV.first);
		if (Operand->ft->type_attr & A_atomic) {
			if (Opcode[0] == '+')
				return handle(target, CreateAtomicRMW(llvm::AtomicRMWInst::BinOp::FAdd, OperandV.second, One), Loc, ft);
			else
				return handle(target, CreateAtomicRMW(llvm::AtomicRMWInst::BinOp::FSub, OperandV.second, One), Loc, ft);
		}
...
llvm::Value* BinaryExprAST::codegen_raw(llvm::Value* target) {
		...
		if (opclass != OpDeclAssign) {
			if (auto idx_expr = dynamic_cast<IndexExprAST*>(LHS.get())) {
				if (idx_expr->Field->ft->type == llvm_map_type && ft->type == llvm_string_type) {
					...
					auto [ inserter_proto, inserter_fn ] = get_inserter_fn_for_key_type(LHS->Loc, idx_expr->Field->ft);
					...
					Builder->CreateCall(inserter_proto->FT, inserter_fn, std::vector<llvm::Value*>{
							ptr, Key, cstr_val_int, sz, do_replace });
					// Check if  we have to increase the count
					llvm::Value* old_elem = Builder->CreateLoad(llvm_ptr_type, do_replace);
					llvm::Value* int_value = Builder->CreatePtrToInt(old_elem, llvm_size_type);
					...
					// We have replaced an existing element (free it?)
					Builder->CreateBr(MergeBB);
					if (TheFunction) {
						TheFunction->insert(TheFunction->end(), NullPtr);
						Builder->SetInsertPoint(NullPtr);
					}
					llvm::Value* count_ref = Builder->CreateStructGEP(map_type, map_ref, 1);
					llvm::Value* count = Builder->CreateLoad(llvm_size_type, count_ref);
					count = Builder->CreateAdd(count, getSize(1));
					Builder->CreateStore(count, count_ref);
					Builder->CreateBr(MergeBB);
					if (TheFunction) {
	...
	not_found:
		...
		unsigned attribs = RHS->ft->type_attr & (A_signed | A_cstring | A_closure | A_constructor | A_destructor);
...
bool ForExprAST::PrepareIterator() {
	...
	case existing_var_returned:
	case generic_lvalue_returned:
		auto KeyLval = dynamic_cast<LvalueExprAST*>(Key.get());
		if (!KeyLval) {
...
llvm::Value* BranchExprAST::codegen_raw(llvm::Value* target) {
	...
	if (ft->type->isVoidTy())
		...
		if (CTcond == CTcond_true)
			return handle(target, ThenV, Loc, ft);
		else if (CTcond == CTcond_false)
			return handle(target, ElseV, Loc, ft);
		...
		return handle(target, merge.second, Loc, ft);
	}
}
