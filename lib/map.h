...
// return address of inserted node with lowest bit set if existing node has been replaced
		...
		_DECL Node* string_insert(Node** root_ptr, const char* key, Value value, int value_size, bool allow_replace);
		_DECL Node* string_tag_insert(Node** root_ptr, const char* key, unsigned tag, Value value, int value_size, bool allow_replace);
		_DECL Node* u64_insert(Node** root_ptr, u64 key, Value value, int value_size, bool allow_replace);
		_DECL Node* i64_insert(Node** root_ptr, i64 key, Value value, int value_size, bool allow_replace);
		_DECL Node* u32_insert(Node** root_ptr, u32 key, Value value, int value_size, bool allow_replace);
		_DECL Node* i32_insert(Node** root_ptr, i32 key, Value value, int value_size, bool allow_replace);
